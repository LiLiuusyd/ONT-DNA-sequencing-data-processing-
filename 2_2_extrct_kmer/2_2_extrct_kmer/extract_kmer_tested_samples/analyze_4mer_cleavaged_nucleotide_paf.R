...
# ---- User Inputs ----
paf_file <- "/srv/scratch/z5395183/ProjectGa/20250506_demux_fastq/demu_SQK-NBD114-96_barcode28.paf"
ref_file <- "/srv/scratch/z5395183/ProjectGa/analysis/pUC19.fasta"
output_file <- "/srv/scratch/z5395183/ProjectGa/20250506_demux_fastq/demu_barcode28_4mer.csv"
...
# ---- 4-mer Extraction (two specific windows) ----
get_4mer_window <- function(pos, ref, offset_start, offset_end) {
  ref_len <- nchar(ref)
...
# Define the two windows as offsets relative to the cleavage site
# (offset_start, offset_end)
fourmer_windows <- list(
  window1 = c(-2, 1),    # pos-2 to pos+1
  window2 = c(-1, 2)     # pos-1 to pos+2
)

# Helper to extract all windows for a set of positions
extract_4mers_all_windows <- function(positions, ref) {
  lapply(fourmer_windows, function(offsets) {
    sapply(positions, function(pos) get_4mer_window(pos, ref, offsets[1], offsets[2]))
  })
}

# For positive strand
start_4mers_pos_all <- extract_4mers_all_windows(valid_starts_pos, ref)
end_4mers_pos_all   <- extract_4mers_all_windows(valid_ends_pos, ref)
# For negative strand
start_4mers_neg_all <- extract_4mers_all_windows(valid_starts_neg, ref)
end_4mers_neg_all   <- extract_4mers_all_windows(valid_ends_neg, ref)

# Only count AAAA, TTTT, CCCC, GGGG
fourmer_targets <- c("AAAA", "TTTT", "CCCC", "GGGG")
count_4mers <- function(fourmers, targets) {
  fourmers <- fourmers[!is.na(fourmers)]
  tab <- table(factor(fourmers, levels = targets))
  total <- sum(tab)
...
results <- list(
  pos_start = lapply(start_4mers_pos_all, count_4mers, targets = fourmer_targets),
  pos_end   = lapply(end_4mers_pos_all, count_4mers, targets = fourmer_targets),
  neg_start = lapply(start_4mers_neg_all, count_4mers, targets = fourmer_targets),
  neg_end   = lapply(end_4mers_neg_all, count_4mers, targets = fourmer_targets)
)

# ---- Output ----
window_names <- c(
  "Window 1: pos-2 to pos+1",
  "Window 2: pos-1 to pos+2"
)
strand_pos_names <- c("Positive strand, start", "Positive strand, end", "Negative strand, start", "Negative strand, end")

cat("\n================ Cleavage 4-mer Extraction Results ================\n")
for (strand_pos in names(results)) {
  cat("\n---", gsub("_", ", ", strand_pos), "---\n")
  for (i in seq_along(window_names)) {
    res <- results[[strand_pos]][[i]]
    cat(window_names[i], "\n")
    cat("Total 4-mers:", res$total, "\n")
    print(data.frame(Fourmer = fourmer_targets, Frequency = as.integer(res$counts), Proportion = round(res$proportions, 4)))
    cat("\n")
  }
}

# ---- Summarize by strand (sum both windows for each strand) ----
sum_4mer_counts <- function(res_list) {
  # res_list: list of 2 window results for a strand (each is a list with counts)
  Reduce(`+`, lapply(res_list, function(x) as.integer(x$counts)))
}

# Sum for positive and negative strands
pos_total_counts <- sum_4mer_counts(c(results$pos_start, results$pos_end))
neg_total_counts <- sum_4mer_counts(c(results$neg_start, results$neg_end))

# Prepare summary data frame
summary_df <- data.frame(
  Fourmer = fourmer_targets,
